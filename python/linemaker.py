...
, seed=21)
    print(f"\nFinal: LAD {line.current_price_a:+d}  |  SDP {line.current_price_b:+d}")
    print(f"Handle: LAD ${line.handle_a:.0f}  |  SDP ${line.handle_b:.0f}  (imbalance: {imbalance_pct(line):.1f}%)")
    print(f"Exposure: {book_exposure(line)}")
    print(f"Total moves: {len(line.line_moves)}")
    write_linemaker_demo()
emaker_demo()
 LAD ${line.handle_a:.0f}  |  SDP ${line.handle_b:.0f}  (imbalance: {imbalance_pct(line):.1f}%)")
    print(f"Exposure: {book_exposure(line)}")
    print(f"Total moves: {len(line.line_moves)}")
    write_linemaker_demo()
emaker_demo()
