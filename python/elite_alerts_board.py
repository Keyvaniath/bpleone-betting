...
def run() -> Dict[str, Any]:
    sources = {
        ...
        "UFC_FIGHTER_DOMINANCE": (
            _load(os.path.join(DATA_DIR, "ufc_fighter_dominance.json")),
            "alerts", "fighter", "n_signals", "fight",
        ),
        "GOLF_PLAYER_DOMINANCE": (
            _load(os.path.join(DATA_DIR, "golf_player_dominance.json")),
            "alerts", "player", "n_signals", "tournament",
        ),
        "WNBA_PLAYER_CEILING": (
            _load(os.path.join(DATA_DIR, "wnba_player_ceiling_stack.json")),
            "alerts", "player", "n_aligned_overs", "matchup",
        ),
        "F1_DRIVER_DOMINANCE": (
            _load(os.path.join(DATA_DIR, "f1_driver_dominance.json")),
            "alerts", "driver", "n_signals", "race",
        ),
        "NBA_TEAM_EXPLOSION": (
            _load(os.path.join(DATA_DIR, "nba_team_explosion_alerts.json")),
            "alerts", "team", "n_positive_signals", "matchup",
        ),
        "NHL_TEAM_EXPLOSION": (
            _load(os.path.join(DATA_DIR, "nhl_team_explosion_alerts.json")),
            "alerts", "team", "n_positive_signals", "matchup",
        ),
        "MLB_BATTER_EXPLOSION": (
            _load(os.path.join(DATA_DIR, "mlb_batter_explosion_alerts.json")),
            "alerts", "batter", "n_aligned_overs", "matchup",
        ),
    }
