...
def run() -> Dict[str, Any]:
    ...
    payload = {
        ...
        "disclosures": [
            ...
            "ROOT CAUSE FOUND 2026-09-08: the lineup pool excluded statuses (O, OUT, IR) borrowed from football, but BASEBALL USES 'IL'. DraftKings was flagging injured players and the optimizer seated them anyway -- 40 IL players on the 9/08 slate, 34 of them seatable. That is the bulk of the projected-vs-actual gap: receipts averaged 3-6 dead slots per lineup. IL/NA/SUSP are now excluded (DTD is not -- those players usually do play). Slates BEFORE 2026-09-08 carry the old, worse lineups and are left exactly as scored; expect the bias to compress from here, and judge the fix on slates dated 9/09 onward.",
