# EdgeStat Daily Brief - 2026-07-24

**Model Confidence: 23.1/100 [RED]** -- Calibration warming up. Research signal only; no real-money sizing yet.

_Pipeline health: **WARNING** (13/15 artifacts ok; 2 empty, 0 stale)._ 

_Generated at 2026-07-24T23:08:30 UTC. Game lines source: **Bovada (fallback -- DK primary unavailable)**. Pick-em opportunities from PrizePicks._

## Play of the Day

**OAK @ MIN - OVER_10.5**
- Market: -110
- Model probability: 80.8%
- Raw edge: +54.32%
- Recommended stake: <= 0.5u (model calibrating)

> _Edge >= 15% is well above what a properly-calibrated baseball model produces. Treat this as a research signal until 30+ outcomes have flowed through the calibration loop._

## Full Slate (9 games)

| Time | Matchup | Park | Weather | Top edge |
|---|---|---|---|---|
| 7:10p ET | LAD @ NYM | Citi Field | 68F 4mph | OVER_8.0 +29.25% |
| 7:10p ET | CLE @ TBR | Tropicana Field | indoor | UNDER_8.0 +18.7% |
| 7:10p ET | SDP @ MIA | loanDepot park | indoor | MIA_ML +43.96% |
| 7:15p ET | TOR @ BOS | Fenway Park | 69F 5mph | BOS_ML +32.8% |
| 7:40p ET | HOU @ CHW | Rate Field | 67F 6mph | CHW_ML +28.53% |
| 8:05p ET | SEA @ TEX | Globe Life Field | indoor | -- |
| 8:10p ET | OAK @ MIN | Target Field | 74F 9mph | OVER_10.5 +54.32% |
| 8:15p ET | CIN @ STL | Busch Stadium | 69F 3mph | STL_ML +14.71% |
| 10:15p ET | LAA @ SFG | Oracle Park | 59F 16mph | SFG_ML +23.7% |

## Parlays - top 5

- **2-leg @ +240 (prob 44.0%, EV +49.78%)**
  - ATL @ BAL OVER_9.0 (-110, model 63.9%)
  - LAA @ SFG SFG_ML (-128, model 68.9%)
- **2-leg @ +214 (prob 47.5%, EV +49.33%)**
  - HOU @ CHW CHW_ML (-131, model 69.0%)
  - LAA @ SFG SFG_ML (-128, model 68.9%)
- **2-leg @ +237 (prob 44.1%, EV +48.36%)**
  - ATL @ BAL OVER_9.0 (-110, model 63.9%)
  - HOU @ CHW CHW_ML (-131, model 69.0%)
- **2-leg @ +231 (prob 44.5%, EV +47.24%)**
  - NYY @ PHI NYY_ML (-119, model 64.7%)
  - LAD @ NYM LAD_ML (-125, model 68.7%)
- **2-leg @ +244 (prob 42.7%, EV +46.88%)**
  - LAD @ NYM LAD_ML (-125, model 68.7%)
  - CLE @ TBR UNDER_8.0 (-110, model 62.2%)

## Self-Learning Loop

| Market | n settled | Hit rate | Model implied | Bias | Correction |
|---|---|---|---|---|---|

Cumulative graded plays: 6525. Wins: 2681. Hit rate: 41.1%.

## Auxiliary Markets (Model Fair Prices)

| Matchup | NRFI % | NRFI fair | F5 total | RL home -1.5 fair | RL away +1.5 fair |
|---|---|---|---|---|---|
| ATL @ BAL | 54.4% | -119 | 3.04 | -- | -- |
| CIN @ STL | 30.2% | +231 | 5.94 | +105 | -105 |
| CLE @ TBR | 53.0% | -113 | 3.17 | -116 | +116 |
| HOU @ CHW | 31.2% | +220 | 6.03 | -135 | +135 |
| LAA @ SFG | 25.0% | +300 | 7.63 | -131 | +131 |
| LAD @ NYM | 33.7% | +197 | 5.49 | +384 | -384 |
| OAK @ MIN | 19.5% | +413 | 8.51 | -100 | +100 |
| SDP @ MIA | 41.8% | +139 | 4.36 | -253 | +253 |
| SEA @ TEX | 32.8% | +205 | 5.58 | +165 | -165 |
| TOR @ BOS | 38.5% | +160 | 4.91 | -135 | +135 |

## Travel / Rest Flags

- **ARI @ WSN** (home): 2 days rest (+2h tz)
- **SEA @ TEX** (away): 2 days rest (+2h tz)
- **CIN @ STL** (away): 2 days rest (+2h tz)
- **LAA @ SFG** (home): 2 days rest (-2h tz)

## Team Form (last 10)

**Hot:** CWS 7-3 (W1, +30), AZ 8-2 (W3, +29), BOS 9-1 (W1, +27), BAL 8-2 (L1, +20), WSH 5-5 (W1, +19)

**Cold:** ATH 2-8 (L2, -41), TOR 3-7 (W1, -26), KC 5-5 (L1, -22), COL 3-7 (L1, -21), MIA 1-9 (L9, -16)

## Gassed Bullpens (> 8.0 IP in 2 days)

- TB: 8.3 IP across 2 games
- TOR: 8.5 IP across 2 games
- MIN: 10.0 IP across 2 games
- BOS: 11.2 IP across 2 games

## Loop Activity (since last refresh)

- Confidence delta: **+0.0**

## Model Recommendations (operator review)

_The model is suggesting 1 parameter tweak based on its own performance. Apply via `data/runtime_config.json` on `/config`._

- **[MEDIUM] `live_edges.edge_threshold_pp`** ↓ 3.5 -> **2.0**
  - _Zero live edge alerts in last 24h despite live props being priced. Threshold may be too tight to surface anything actionable._

---

_EdgeStat is a research desk. Bet responsibly. 21+. 1-800-GAMBLER._
_Source: github.com/Keyvaniath/bpleone-betting - last settled 2026-07-23._