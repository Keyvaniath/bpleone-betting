# Tonight's Slate Brief — 2026-07-21

## Slate Quality: 55/100 (MEDIUM)
_0% lineups confirmed · 15 rookie SPs_
**Recommendation:** Slightly smaller bets; skip pre-cal edges (>15%).

## Slate
- **15 games** scheduled
- **24 best bets** scored: 0 DK · 0 PP · 12 NRFI · 0 SGP
- **Auto-portfolio:** 2 picks, $100.0 stake (10.0%), expected +$50.89
- **MC simulation:** EV +$129.31, 84% P(profit), p05 $-91.28 / p95 $326.25

## Top 5 Best Bets
1. **OAK @ ARI YRFI (1st-inning OVER 0.5)**  (80/100 ****, edge +33.8%)
   - P(YRFI) 79.8% vs baseline 46.0%
2. **SDP @ ATL YRFI (1st-inning OVER 0.5)**  (80/100 ****, edge +32.3%)
   - P(YRFI) 78.3% vs baseline 46.0%
3. **WSN @ COL YRFI (1st-inning OVER 0.5)**  (80/100 ****, edge +32.2%)
   - P(YRFI) 78.2% vs baseline 46.0%
4. **MIA @ HOU YRFI (1st-inning OVER 0.5)**  (79/100 ****, edge +19.7%)
   - P(YRFI) 65.7% vs baseline 46.0%
5. **PIT @ NYY YRFI (1st-inning OVER 0.5)**  (79/100 ****, edge +19.5%)
   - P(YRFI) 65.5% vs baseline 46.0%

## ⚠️ Trap Warnings — 12 flagged
- **OAK @ ARI YRFI (1st-inning OVER 0.5)** (low):
   - Edge +33.8% looks big but only 0 settled props on this player — model unproven
- **SDP @ ATL YRFI (1st-inning OVER 0.5)** (low):
   - Edge +32.3% looks big but only 0 settled props on this player — model unproven
- **WSN @ COL YRFI (1st-inning OVER 0.5)** (low):
   - Edge +32.2% looks big but only 0 settled props on this player — model unproven
- **MIA @ HOU YRFI (1st-inning OVER 0.5)** (low):
   - Edge +19.7% looks big but only 0 settled props on this player — model unproven
- **PIT @ NYY YRFI (1st-inning OVER 0.5)** (low):
   - Edge +19.5% looks big but only 0 settled props on this player — model unproven

## Bullpens GASSED tonight
Athletics, Pittsburgh Pirates, San Diego Padres, Seattle Mariners, San Francisco Giants, St. Louis Cardinals, Tampa Bay Rays, Minnesota Twins
_Lean late-inning OVER on these games; fade their save situations._

## NRFI/YRFI strong leans
- **YRFI** (1st-inning OVER): OAK @ ARI, SDP @ ATL, WSN @ COL, MIA @ HOU, PIT @ NYY

## NBA -- PRESEASON
- Memphis Grizzlies @ Atlanta Hawks (0-0 vs 0-0)  | P(home) 55.0% fair -122 | Scheduled

## NHL -- PRESEASON
- Montreal Canadiens @ Toronto Maple Leafs (0-0-0 vs 0-0-0)  | P(home) 52.9% fair -112 | Scheduled
- Toronto Maple Leafs @ Montreal Canadiens (0-0-0 vs 0-0-0)  | P(home) 52.9% fair -112 | Scheduled
- Dallas Stars @ St. Louis Blues (0-0-0 vs 0-0-0)  | P(home) 52.9% fair -112 | Scheduled
- Winnipeg Jets @ Edmonton Oilers (0-0-0 vs 0-0-0)  | P(home) 52.9% fair -112 | Scheduled
- Chicago Blackhawks @ Minnesota Wild (0-0-0 vs 0-0-0)  | P(home) 52.9% fair -112 | Scheduled

## KBO -- 2026 Korean Baseball
- Hanwha Eagles @ KIA Tigers | P(home) 72.8% fair -267 | total 9 R
- NC Dinos @ LG Twins | P(home) 63.1% fair -171 | total 9 R
- Doosan Bears @ KT Wiz | P(home) 49.8% fair +101 | total 9 R
- SSG Landers @ Lotte Giants | P(home) 45.8% fair +118 | total 9 R
- Samsung Lions @ Kiwoom Heroes | P(home) 33.2% fair +201 | total 9 R
- **KBO POD:** LG Twins ML vs NC Dinos (KBO) (HIGH)

## LoL Esports -- 0 live, 4 upcoming
- [LPL] LGD GAMING vs EDWARD GAMING BO3 | P(A) 50.0% fair -100/-100
- [LPL] TOP ESPORTS vs Xi'an Team WE BO3 | P(A) 42.4% fair +136/-136
- [NLC] DMG Esports vs Epic Avalanche BO3 | P(A) 50.0% fair -100/-100
- [NLC] Ruddy Corporation vs Rich Gang BO3 | P(A) 50.0% fair -100/-100
- **LoL POT:** Xi'an Team WE ML vs TOP ESPORTS (LPL, BO3) (HIGH)

## Player Play of the Day -- across all sports (3645 candidates scanned)
- **[LOL]** Inspired UNDER 3.5 Kills | model 61.7% fair -161 (ELITE)
- **[LOL]** ShowMaker OVER 3.5 Kills | model 62.5% fair -167 (ELITE)
- **[LOL]** Peyz OVER 4.5 Kills | model 61.3% fair -158 (ELITE)
- **[LOL]** Hope OVER 4.5 Kills | model 61.3% fair -158 (ELITE)
- **[CS]** torzsi OVER 40.5 Kills (BO3) | model 61.0% fair -157 (ELITE)

## Self-learning calibration (esports + KBO)
- **LOL:** 34 settled | brier 0.2587 | ECE 0.1367
   - Model over-confident; subtract 11.9pp at next inference
- **KBO:** 24 settled | brier 0.3047 | ECE 0.3095
   - Model over-confident; subtract 30.9pp at next inference
